...
- Distribution is automatic: `lightsound/agent-rules/.github/workflows/sync.yml` runs `sync --all` on every push to its `main` that touches `packs/**` or `subscriptions.json`, and on `workflow_dispatch` with a dry-run input (D23). Run `sync --all` by hand only to retry a failed run or to sync from an unmerged pack ref
