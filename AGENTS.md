...
- `src/report/` rendering of a `ScanReport` (`render.ts`) and sync results (`sync.ts`: one target, or the `--all` table) to text; `failure.ts` prints expected failures (refusal, bad `--packs`, GitHub error, an incomplete `--all` run) as one stderr line with exit code 1
