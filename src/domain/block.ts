...
/**
 * Whether a file-level marker inside a region names the region's owner: some token of the region
 * name (split on `:`, `-`, `_`, spaces; three characters or more) appears in the marker text.
 * `<!-- Managed by solid2-agent-kit v0.11.1 … -->` inside `solid2-agent-kit:agents-section` does;
 * `<!-- Generated by CoolGen -->` inside `setup-hooks` does not and keeps its blocking power.
 */
function namesOwner(region: Marker, marker: Marker): boolean {
  const text = marker.raw.toLowerCase();
  return region.key
    .split("-")
