...
separate M1 item, not part of this plan.
...
| Rules packs | Subscribe to `base` and `personal` of `lightsound/agent-rules` | One pull request against `lightsound/agent-rules` adding `"lightsound/rulefleet"` under both keys of `subscriptions.json`. The D23 workflow then opens the block pull request into `rulefleet`, provided `RULECHECK_TOKEN` exists and lists `lightsound/rulefleet` in its repository access (D23; a missing entry reads `failed`, 404). While the secret is missing, run `bun run dev sync lightsound/rulefleet --pack base --packs lightsound/agent-rules` (then `--pack personal`) from a rulecheck checkout; `--dry-run` first |
...
| `RULECHECK_TOKEN` in `lightsound/agent-rules` extended to `lightsound/rulefleet` | agent-rules repository secret (already defined by D23) | Fine-grained PAT; add `rulefleet` to its repository list so the D23 workflow can open the block pull request |
