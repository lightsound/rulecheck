...
| `@path` import position | the line is resolved wherever it sits in the file, between other text or between another tool's single-line HTML-comment markers; "Import parsing skips Markdown code spans and fenced code blocks". Basis of D16: an `@AGENTS.md` line makes `AGENTS.md` load whatever else the root `CLAUDE.md` holds; the same line in a fence loads nothing | [memory docs](https://code.claude.com/docs/en/memory), "CLAUDE.md imports" |
| `@path` inside a multi-line HTML comment | **inferred, not verified**: the docs say "Block-level HTML comments in CLAUDE.md files are stripped before the content is injected" but do not order stripping against import parsing. rulecheck treats such a line as *not* an import; if the inference is wrong the pair reads `both-full` and D12 rewrites `CLAUDE.md` into the wrapper, still a correct end state, whereas the opposite mistake would leave `AGENTS.md` unloaded. Verify with the probe method below (an import inside a comment, a marker in the imported file) | docs (stripping only); the ordering is an inference |
