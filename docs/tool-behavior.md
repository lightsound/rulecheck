...
|---|---|---|
| Public API | none. The Admin API (Teams/Enterprise) exposes members, usage, spend, audit logs, billing groups, model access. `team_rule` appears only as an audit-log event type; there is no endpoint that reads or writes Team Rules or User Rules | [Admin API](https://cursor.com/docs/account/teams/admin-api) |
