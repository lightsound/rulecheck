...
- Status precedence per repository per pack: a block with `source=<pack>` in the root
  `AGENTS.md` / `CLAUDE.md` decides (`modified` if the body hash differs from `hash=`, else
  `outdated` if `hash=` differs from the pack's current hash, else `current`), regardless of
  subscription. Without a block: `not subscribed`, or for subscribers `blocked` (`both have
  content`, or a foreign/malformed marker in a root file the sync would write) else `eligible`.
  Comparison is by hash, not `rev`: a pack commit that touches other files leaves subscribers
  `current`.
...
Skills (D8) get no finding for a hash mismatch against `skills-lock.json`: `npx skills` may record
a server-side snapshot hash rather than the folder hash, so a mismatch does not prove drift. The
state is shown per skill (`matches lock`, `lock hash differs`, `no lock entry`); the one finding
is a lock entry whose directory is missing and not gitignored.
