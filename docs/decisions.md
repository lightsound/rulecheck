...
- Status precedence per repository per pack: a block with `source=<pack>` in the root
  `AGENTS.md` / `CLAUDE.md` decides, regardless of subscription. If the file carrying the block
  also has a malformed or foreign marker the row is `blocked` (a rewrite would land inside a file
  another tool or a broken marker owns; the message still names the block's own state), else
  `modified` if the body hash differs from `hash=`, else `outdated` if `hash=` differs from the
  pack's current hash, else `current`. Without a block: `not subscribed`, or for subscribers
  `blocked` (`both have content`, or a foreign/malformed marker in a root file the sync would
  write) else `eligible`. Every write, insertion or update, is thus gated by "a human looks
  first" (D6). Comparison is by hash, not `rev`: a pack commit that touches other files leaves
  subscribers `current`.
