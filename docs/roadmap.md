...
- Result: `src/domain/block.ts`, `src/domain/skills.ts`, `src/domain/pack.ts` (pure, tested),
  ...
  `modified`. Skills: `skills-lock.json` `computedHash` is not reliably reproducible from disk
  (`npx skills` may store a server-side snapshot hash; an installed skill identical to upstream
  still differed from its lock entry), so hash differences are shown as a per-skill state, not a
  finding. Missing lock entries are findings unless the skill directory is gitignored. The
